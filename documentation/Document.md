...
Требуется создать рабочий прототип Web приложения для работы с пользователями. Приложение должно реализовывать следующие
функции:
...
Данный режим программы предназначен для просмотра списка апартаментов. Основной сценарий:
...
Данный режим программы предназначен для просмотра списка постояльцев. Основной сценарий:

- Пользователь выбирает меню «Residents».
- Отображается форма просмотра списка всех постояльцев.

![Рис. 3.1 Просмотр списка постояльцев](doc_photos/Cut_resident_with_search.jpg)
...
## 3.5. Поиск постояльца по дате

#### Основной сценарий:

- Пользователь находясь в режиме просмотра списка постояльцев вводит "Arrival time", "Departure time" и нажимает кнопку
  «Search»;
- Отображается список постояльцев сортированных по "Arrival time", у которых "Arrival time" больше или равно введеному "
  Arrival time" и "Departure time" меньше или равно введенному "Departure time";

![Рис. 3.5.1 Просмотр списка постояльцев](doc_photos/Cut_resident_with_search_and_date.jpg)
Рис. 3.5.1 Поиск постояльца по дате

#### Ошибочный сценарий:

- Если удовлетворяющих критериям поиска постояльцев нет, то отображается пустая страница:

![Рис. 3.5.2 Просмотр списка постояльцев не удовлетворяющих критериям поиска](doc_photos/Cut_search_without_result.jpg)
Рис. 3.5.2 Поиск постояльцев не удовлетворяющих критериям поиска

- Если пользователь вводит "Arrival time" со значением превышающим "Departure time", то отображается страница ошибки:

![Рис. 3.5.2 Просмотр списка постояльцев с неверным условием поиска](doc_photos/Cut_error_page.jpg)
Рис. 3.5.2 Поиск постояльцев с неверным условием поиска
